...
const Contents_List_all = () => {
  ...
  let contents_page = parseInt(sessionStorage.getItem("contents_page"));
  console.log('-> contents_page:', contents_page);
  console.log('-> contents_page?contents_page:0:', contents_page?contents_page:0);
  ...
  const [page, setPage] = useState(contents_page?contents_page:0); // 현재 페이지, 0-base
  ...
  const load = async (p) => {
    sessionStorage.setItem("contents_page", p);

    const res = await axiosInstance.get('/contents/list_all_paging', { params: { page: p, size, cateno } });
