...
const Cate_List = () => {
  ...
  const [delete_panel, setDelete_panel] = useState(false); // 삭제 패널
  const read_for_delete = (cateno) => {
    setDelete_panel(true); // 삭제 패널 출력
                ...
                data && data.map((item, index) => 
                  <tr key={index}>
                    ...
                    <td className='table_underline' style={{textAlign: 'center'}}>
                      {item.cateno} 
                      {
                        item.visible === 'Y' ? (<img src={show_img} className='icon' />) 
                        : 
                        (<img src={hide_img} className='icon' />)
                      }
                      <a href='#' onClick={() => read_for_update(item.cateno)}><img src={update_img} className='icon' /></a>
                      <a href='#' onClick={() => read_for_delete(item.cateno)}><img src={delete_img} className='icon' /></a>
                    </td>
                  </tr>
                )
              )
